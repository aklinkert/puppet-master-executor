...
import { headless, puppeteerConfig, queueNameJobs, queueConfig } from './config';
...
async function handleJob(job) {
    ...
    try {
        ...
        const { logs, error } = await executeJob(browser, logger, job);

        logger.debug(`Closing browser`);
        await browser.close();

        logger.debug(`Done executing task`);
        logger.info(`Error: ${error}, Logs: ${logs.join(" \\n ")}`);
    } catch (e) {
        logger.error(e);
        throw e;
    } finally {
